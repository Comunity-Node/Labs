...
contract CoFinance is ReentrancyGuard {
    ...
    function borrowTokens(uint256 amount, uint256 duration) external hasCollateral {
        ...
        if (borrowedToken[msg.sender] == address(tokenA)) {
            collateralAmount = collateralA[msg.sender];
            require(amount <= collateralAmount * 80 / 100, "Borrow amount exceeds 80% of collateral");
            collateralA[msg.sender] -= amount * 2;
            totalCollateralA -= amount * 2;
        } else {
            collateralAmount = collateralB[msg.sender];
            require(amount <= collateralAmount * 80 / 100, "Borrow amount exceeds 80% of collateral");
            collateralB[msg.sender] -= amount * 2;
            totalCollateralB -= amount * 2;
        }

        borrowed[msg.sender] += amount;
        loanStartTime[msg.sender] = block.timestamp;
        loanDuration[msg.sender] = duration;
        borrowedToken[msg.sender] = address(tokenA); // Example logic, adjust as needed

        // Lock the collateral in CollateralLockPool
        if (amount * 2 != 0) {
            IERC20(borrowedToken[msg.sender]).safeTransfer(address(collateralLockPool), amount * 2);
            collateralLockPool.lockCollateral(msg.sender, borrowedToken[msg.sender], amount * 2);
        }
    ...
    function distributeInterestFees(uint256 amount) internal {
        ...
        uint256 poolShare = amount - ownerShare;
